...
up_limit <- 0.7 ; down_limit <- 0.8   ##   up_limit <- 0.67 ; down_limit <- 0.52
...
# remove drugs with too many or too few targets: NOT reliable.
subset <- subset[ subset[ ,3]>1 , ]
subset <- subset[ subset[ ,3]<3 , ]
...
target <- read.csv("/Users/miyang/Documents/RWTH_Aachen/macau_work_dir/macau_test_sanger/DATA/target", check.names = F) 
target <- target[!duplicated(target[,1]),] ; rownames(target) <- target[,1] ; target <- target[ ,-1]
combo_coread <- readRDS("/Users/miyang/Documents/RWTH_Aachen/SANGER_DATA/DATA_COMBO/gdscCOREAD_dataset.RDS") 
...
x <- retrieve_interaction_plot(drug_feature_name="target",cell_feature_name="progeny11",selection="conservative",cut_off="single",tissue_name = tissue)[[1]] ; x <- x[[tissue_number]]
target_SANGER <- rownames(x)
target_combo <- read.csv("~/Documents/RWTH_Aachen/DREAM_AZ/DATA_MERCK/target_combo", row.names=1, check.names = F)
