...
ComboCompoundNames_all <- read.delim("~/Documents/RWTH_Aachen/DREAM_AZ/DATA_ALMANAC/ComboCompoundNames_all.txt", header=FALSE)
ComboCompoundNames_all[,2] = toupper(ComboCompoundNames_all[,2])

ALMANAC <- read.csv(paste0(path2,"ComboDrugGrowth_Nov2017.csv"))
...
target_GDSC <- read.csv(paste0(path,"/SANGER_DATA/FEATURES/target") ,check.names = F) ; target_GDSC <- target_GDSC[ ,-1]
...
corr_table <- corr_table[ -which(rownames(corr_table)=="EGFR/ERBB2") , ] # remove this, because the 1 drug targets ERBB2/EGFR, 1 drug target ERBB2, so there is no synergy
...
# average_synergy
# corr_table_candidate_subset

for( i in 1:length(colnames(corr_table_candidate_subset))) {
  
  df <- cbind(corr_table_candidate_subset[ ,i],average_synergy[ ,i])
  colnames(df) <- c("similarity","synergy")
  df <- as.data.frame(df)
  df <- aggregate(similarity~synergy, df, mean)
  df <- cbind(df$similarity,df$synergy) 

  library(cowplot)
  pdf(file=paste0(result_folder,"/","COMBO_",colnames(corr_table_candidate_subset)[i],".pdf"), width = 40, height = 10,  compress=TRUE, onefile = F)
  par(mfrow=c(1,4))
  
  plot_List <- list()
  df_neg <- df[df[ ,1] <= -0.3 , ]
  df_pos <- df[df[ ,1] >= 0.3 , ]
  df_mid <- df[rownames(df) %not in% c(rownames(df_neg),rownames(df_pos)), ]
  df_tail_abs <- df[rownames(df) %not in% rownames(df_mid), ] ; df_tail_abs[ ,1] <- abs(df_tail_abs[ ,1])
  df_abs <- df  ;  df_abs[ ,1] <-  abs(df_abs[ ,1])
  
  synergy_label <- "Number of synergistic cases"
  
  plot_List[[1]] <- scatter_plot_normal(df=df_neg,title=paste0("Target pairs (",colnames(corr_table_candidate_subset)[i],")"),x_lab="Target functional similarity",y_lab=synergy_label,coeff_x=0.25)
  plot_List[[2]] <- scatter_plot_normal(df=df_mid,title=paste0("Target pairs (",colnames(corr_table_candidate_subset)[i],")"),x_lab="Target functional similarity",y_lab=synergy_label,coeff_x=0.5)
  plot_List[[3]] <- scatter_plot_normal(df=df_pos,title=paste0("Target pairs (",colnames(corr_table_candidate_subset)[i],")"),x_lab="Target functional similarity",y_lab=synergy_label,coeff_x=0.7)
  plot_List[[4]] <- scatter_plot_normal(df=df_abs,title=paste0("Target pairs (",colnames(corr_table_candidate_subset)[i],")"),x_lab="Absolute Target functional similarity",y_lab=synergy_label,coeff_x=0.7)
  
  print( plot_grid(plot_List[[1]],plot_List[[2]],plot_List[[3]],plot_List[[4]], nrow = 1, ncol = 4 , scale = 0.85) )
